...
float4 main(PixelShaderInput pin) : SV_Target
{
 ...
 //   }
    ...
    if (luma > 0.95)
        directLighting = float3(1, 1, 1) * albedo;
    else if (luma > 0.5)
        directLighting = float3(0.75, 0.75, 0.75) * albedo;
    else if (luma > 0.05)
        directLighting = float3(0.35, 0.35, 0.35) * albedo;
    else
        directLighting = float3(0.001, 0.001, 0.001) * albedo;
    ...
    luma = (0.2126 * directLighting.r + 0.7152 * directLighting.g + 0.0722 * directLighting.b);
