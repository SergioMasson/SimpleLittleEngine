...
bool sle::graphics::bloom::Enable = true;
float sle::graphics::bloom::Exposure = 1.8f;
...
namespace sle
{
	namespace graphics
	{
		...
		void CreateMergeTexture(UINT width, UINT height)
		{
			...
			if (bloom::g_inputRenderTarget != nullptr)
				bloom::g_inputRenderTarget.Reset();
		...
		{
			if (g_cutoffTexture != nullptr)
				g_cutoffTexture.Reset();

			if (g_cutoffTextureSRV != nullptr)
				g_cutoffTextureSRV.Reset();

			if (g_cutoffTextureRTV != nullptr)
			...
			ASSERT_SUCCEEDED(hr);
			hr = graphics::g_d3dDevice->CreateRenderTargetView(g_cutoffTexture.Get(), 0, g_cutoffTextureRTV.GetAddressOf());
			ASSERT_SUCCEEDED(hr);
			hr = graphics::g_d3dDevice->CreateShaderResourceView(g_cutoffTexture.Get(), 0, g_cutoffTextureSRV.GetAddressOf());
			ASSERT_SUCCEEDED(hr);
		}

		void CreateBlurTexture(UINT width, UINT height)
		{
			if (g_horizontalBlurTexture != nullptr)
				g_horizontalBlurTexture.Reset();

			if (g_horizontalBlurTextureRTV != nullptr)
				g_horizontalBlurTextureRTV.Reset();

			if (g_horizontalBlurTextureSRV != nullptr)
				g_horizontalBlurTextureSRV.Reset();

			if (g_completeBlurTexture != nullptr)
				g_completeBlurTexture.Reset();

			if (g_completeBlurTextureRTV != nullptr)
				g_completeBlurTextureRTV.Reset();

			if (g_completeBlurTextureSRV != nullptr)
				g_completeBlurTextureSRV.Reset();
			...
			ASSERT_SUCCEEDED(hr);
			hr = graphics::g_d3dDevice->CreateRenderTargetView(g_horizontalBlurTexture.Get(), 0, g_horizontalBlurTextureRTV.GetAddressOf());
			ASSERT_SUCCEEDED(hr);
			hr = graphics::g_d3dDevice->CreateShaderResourceView(g_horizontalBlurTexture.Get(), 0, g_horizontalBlurTextureSRV.GetAddressOf());
			ASSERT_SUCCEEDED(hr);
		...
		void RenderBlur(ID3D11RenderTargetView* const* target)
		{
			...
			//Blur texture 10 time (5 times horizontal 5 times vertical)
			for (size_t i = 0; i < amount; i++)
			{
				...
				pBloom->horizontal = horizontal;
				pBloom->exposure = bloom::Exposure;
				graphics::g_d3dImmediateContext->Unmap(g_pcbBloom.Get(), 0);

				auto renderTarget = horizontal ? g_horizontalBlurTextureRTV.GetAddressOf() : g_completeBlurTextureRTV.GetAddressOf();

				graphics::g_d3dImmediateContext->OMSetRenderTargets(1, renderTarget, nullptr);
				graphics::g_d3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
				graphics::g_d3dImmediateContext->VSSetShader(g_pVertexShaderBloom.Get(), NULL, 0);
				...
				if (first_iteration)
				{
					targetTexture = g_cutoffTextureSRV.GetAddressOf();
				}
				else
				{
					targetTexture = horizontal ? g_completeBlurTextureSRV.GetAddressOf() : g_horizontalBlurTextureSRV.GetAddressOf();
				}

				graphics::g_d3dImmediateContext->PSSetShaderResources(0, 1, targetTexture);
				graphics::g_d3dImmediateContext->Draw(4, 0);
