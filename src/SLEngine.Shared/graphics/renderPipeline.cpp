...
void graphics::RenderPipeline::RenderMesh(MeshRenderer const& mesh)
{
	...
	if (!m_camera->GetWorldFrustum().IntersectBoundingBox(mesh.WBoudingBox()))
		return;
	...
	math::Matrix4 meshWorldMatrix = mesh.GetWorldMatrix().Transpose();

	math::Matrix4 mvpMatrix = (meshWorldMatrix * m_camera->GetViewProjectionMatrix());
